...
clf_yarn = svm.LinearSVC(penalty="l1",dual=False)
clf_yarn.fit(X_train, y_train)
y_test_hat = clf_yarn.predict(X_test)
...
for fold, (train_i, test_i) in enumerate(kf.split(X_pca)):
    ...
    clf_yarn.fit(X_train, y_train)
    y_hat_train = clf_yarn.predict(X_train)
    train_acc = accuracy_score(y_train, y_hat_train)
    acc_train.append(train_acc)
    
    y_hat_test = clf_yarn.predict(X_test)
...
#filename = 'model_yarn.sav'
#pickle.dump(clf_yarn, open(filename, 'wb'))

vectorizor_fname = 'yarn_vectorizor.sav'
