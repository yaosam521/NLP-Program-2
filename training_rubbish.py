...
clf_rubbish = svm.LinearSVC(penalty="l1",dual=False)
clf_rubbish.fit(X_train, y_train)
y_test_hat = clf_rubbish.predict(X_test)
...
for fold, (train_i, test_i) in enumerate(kf.split(X_pca)):
    ...
    clf_rubbish.fit(X_train, y_train)
    y_hat_train = clf_rubbish.predict(X_train)
    train_acc = accuracy_score(y_train, y_hat_train)
    acc_train.append(train_acc)
    
    y_hat_test = clf_rubbish.predict(X_test)
